...
class EmailService:
    ...
    def __init__(self):
        ...
        self.smtp_port = getattr(settings, 'SMTP_PORT', 465)  # По умолчанию используется порт 465
            ...
            try:
                ...
                if self.smtp_port == 465:
                    ...
                    smtp = aiosmtplib.SMTP(
                        ...
                        port=self.smtp_port,
                        use_tls=True,  # Использование TLS
                        tls_context=context
                    )
                    ...
                    smtp = aiosmtplib.SMTP(
                        hostname=self.smtp_host,
                        port=self.smtp_port,
                        use_tls=False,  # Без начального TLS
                        tls_context=context
                    )
                    
                    await smtp.connect()
                    await smtp.starttls()  # Обновление до TLS соединения
                else:
